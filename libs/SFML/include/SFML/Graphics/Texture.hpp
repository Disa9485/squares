...
public:
    ...
    ///
    /// The size of the \a pixel array must match the \a mapWidth and
    /// \a mapHeight arguments, and it must contain 32-bits RGBA pixels.
    ///
    /// No additional check is performed on the size of the pixel
    ...
    /// outside the texture rectangle [0, 0, mapWidth, mapHeight].
    /// In this case, if repeat mode is enabled, the whole texture
    /// will be repeated as many times as needed to reach the
    /// coordinate (for example, if the X texture coordinate is
    /// 3 * mapWidth, the texture will be repeated 3 times).
